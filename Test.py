import cv2
import numpy as np
import utility # IMPORT FUNCTIONS FROM UTILITY FILE

#IMAGE FORMAT#
# path= "Resources/sample.jpg"
path= "Resources/OMR_Custom_Shaded_2.png"
widthImg=600
heightImg=700
#IMAGE FORMAT#
...
# 2.1. BIGGEST CONTOUR (BUBBLE)
biggestContour=utility.getCornerPoints(rectCon[0])
# print(biggestContour.shape)
# 2.2. 2nd BIGGEST CONTOUR (RESULT ZONE)
# gradePoints= utility.getCornerPoints(rectCon[1])
gradePoints= utility.getCornerPoints(rectCon[4])

# 3. LOCATE BUBBLE and RESULT ZONE
if biggestContour.size!=0 and gradePoints.size!=0:
    cv2.drawContours(imgBiggestContours,biggestContour,-1,(0,255,0),20) #HIGHLIGHT BORDER IN GREEN
    cv2.drawContours(imgBiggestContours,gradePoints,-1,(255,0,0),20) #HIGHLIGHT BORDER IN BLUE

    biggestContour=utility.reorder(biggestContour)
    gradePoints=utility.reorder(gradePoints)

    # 3.1. BIRDS EYE VIEW: BUBBLE ZONE
    pt1=np.float32(biggestContour)
    pt2=np.float32([[0,0],[widthImg,0],[0,heightImg],[widthImg,heightImg]])
    matrix=cv2.getPerspectiveTransform(pt1,pt2)
    imgWarpColored=cv2.warpPerspective(img,matrix,(widthImg,heightImg))

    # 3.2. BIRDS EYE VIEW: GRADE ZONE
    ptGrade1=np.float32(gradePoints)
    # ptGrade2=np.float32([[0,0],[325,0],[0,150],[325,150]])
    ptGrade2=np.float32([[0,0],[500,0],[0,150],[500,150]])
    ...
    imgThresh =cv2.threshold(imgWarpGray,170,255,cv2.THRESH_BINARY_INV)[1]    # 170 determines the intensity of the shaded region

    # 3.4. FIND INDIVIDUAL BUBBLES
    # boxes = utility.splitBoxes(imgThresh)
    boxes = utility.splitBoxes(imgThresh)
    # cv2.imshow("Test",boxes[0]) # VIEW SELECTED BUBBLES

    # 3.5. FIND MARK BUBBLES (USE Non-Zero Pixels)
    # print(cv2.countNonZero(boxes[1]),cv2.countNonZero(boxes[2]))

    # 3.6. GETTING NON-Zero PIXEL VALUES OF EACH BUBBLE
    myPixelValue=np.zeros((questions,choices))
    countCol=0
    countRow=0

    # 3.7. Loop through all bubbles
    for image in boxes:
    ...
    # print(myPixelValue) # GET ALL PIXEL VALUES OF BUBBLE

    # 3.8. FINDING INDEX VALUES OF THE MARKING, LOGIC: USE THE MAXIMUM VALUE, TO GET THE SHADED BUBBLE
    myIndex = []
    for x in range (0,questions):
        arr= myPixelValue[x]
        print("Q",x+1,arr)
        myIndexVal=np.where(arr==np.amax(arr))
        print("Detected Answer",myIndexVal[0])
