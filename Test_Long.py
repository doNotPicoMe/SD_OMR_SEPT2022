...
# IMPORT FUNCTIONS FROM utility_long FILE
import utility_long

#########IMAGE#########
# path="sample.jpg"
# path="Resources/OMR_Custom_Shaded_1.png"
path= "Resources/OMR_Long_Short.png"
# ADJUST HERE TO IMRPOVE CLARITY
widthImg=400
heightImg=1000
#########IMAGE#########
...
# FIND RECTANGLES
rectCon=utility_long.rectContour(contours)

#BIGGEST CONTOUR (BUBBLE)
biggestContour=utility_long.getCornerPoints(rectCon[0])
# print(biggestContour.shape)
#2nd BIGGEST CONTOUR (RESULT ZONE)
gradePoints= utility_long.getCornerPoints(rectCon[1])

#LOCATE BUBBLE and RESULT ZONE
if biggestContour.size!=0 and gradePoints.size!=0:
    cv2.drawContours(imgBiggestContours,biggestContour,-1,(0,255,0),20)
    cv2.drawContours(imgBiggestContours,gradePoints,-1,(255,0,0),20)
    biggestContour=utility_long.reorder(biggestContour)
    gradePoints=utility_long.reorder(gradePoints)
    ...
    boxes = utility_long.splitBoxes(imgThresh)
    ...
    imgResults = utility_long.showAnswers(imgResults,myIndex, grading, ans, questions, choices)

    #DISPLAY ANSWERS (Just shades only)
    imgRawDrawing = np.zeros_like(imgWarpColored)
    imgRawDrawing = utility_long.showAnswers(imgRawDrawing,myIndex,grading,ans,questions,choices)
...
imgStacked = utility_long.stackImages(imageArray,0.4,labels)
# DISPLAY FINAL IMAGE
cv2.imshow("Stacked Images",imgFinal)
cv2.imwrite("Test_Long_FinalResult.png", imgFinal)
# cv2.imshow("Stacked Images",imgResults)
