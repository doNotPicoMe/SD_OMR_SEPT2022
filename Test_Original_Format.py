...
# IMPORT FUNCTIONS FROM utility_ori FILE
import utility_ori

#########IMAGE#########
# path="sample.jpg"
path="Resources/OMR_Custom_Shaded_4.png"
widthImg=800
heightImg=800
#########IMAGE#########
...
questions=10
choices=4
# ans = [0,1,2,3,2,1,0,1,2,3,0,1,2,3,2,1,0,1,2,3,0,1,2,3,2,1,0,1,2,3,0,1,2,3,2,1,0,1,2,3]
...
# FIND RECTANGLES
rectCon=utility_ori.rectContour(contours)

#BIGGEST CONTOUR (BUBBLE)
biggestContour=utility_ori.getCornerPoints(rectCon[0])
# print(biggestContour.shape)
#2nd BIGGEST CONTOUR (RESULT ZONE)
gradePoints= utility_ori.getCornerPoints(rectCon[1])
...
if biggestContour.size!=0 and gradePoints.size!=0:
    ...
    biggestContour=utility_ori.reorder(biggestContour)
    gradePoints=utility_ori.reorder(gradePoints)
    ...
    boxes = utility_ori.splitBoxes(imgThresh)
    ...
    imgResults = utility_ori.showAnswers(imgResults,myIndex, grading, ans, questions, choices)

    #DISPLAY ANSWERS (Just shades only)
    imgRawDrawing = np.zeros_like(imgWarpColored)
    imgRawDrawing = utility_ori.showAnswers(imgRawDrawing,myIndex,grading,ans,questions,choices)
...
imgStacked = utility_ori.stackImages(imageArray,0.4,labels)
# DISPLAY FINAL IMAGE
